...
import CostumerReviews from './CostumerReviews.vue'
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(API_URL+`/api/products/read/${slug}`)
      .then((response) => {
  ...
  methods: {
    addToCart() {
      ...
      if (userId) {
        axios.post(API_URL + `/api/orders/create/${userId}/${productId}`)
          .then((response) => {
            console.log('Order created successfully:', response.data);
            alert("You have successfully ordered the product!");
...
<template>
  <div class="container" v-for="product in products" :key="product.id">
    <div class="flex flex-row gap-3 ">
      <div class="w-1/2">
        <img :src="product.image" alt="" class="rounded-2xl my-6" />
      </div>
      <div class="w-1/2 flex flex-col justify-between">
        <p class="font-bold md:text-4xl text-xl mt-4 md:mt-14">{{ product.name }}</p>
        <div class="flex md:flex-row flex-col justify-between ">
          <p class="font-bold md:text-4xl text-xl">{{ '$' + parseFloat(product.price).toFixed(2) }}</p>
          <button
      type="button"
      class="text-gray-900 bg-blue-200 hover:bg-gray-100 border border-gray-500 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 mb-2"
      @click="addToCart"
    >
    <svg
                aria-hidden="true"
